...
exports.createSan = async (req, res) => {
  ...
  try {
    ...
    const isLoaiSanValid = await SanModel.checkLoaiSan(
      connection,
      req.body.loaiSanId,
    );
    if (!isLoaiSanValid) {
    ...
    const isTrung = await SanModel.checkTrungSan(
      connection,
      req.body,
      req.user.id,
    );
    ...
    const sanId = await SanModel.createSan(
      connection,
      req.body,
      diaChiId,
      req.user.id,
...
exports.getAllSan = async (req, res) => {
  try {
    const data = await SanModel.getAllSan();
    res.json(data);
  } catch (err) {
    res.status(500).json(err);
  }
...
exports.searchSan = async (req, res) => {
  try {
    ...
    const data = await SanModel.searchSan(keyword.trim());
